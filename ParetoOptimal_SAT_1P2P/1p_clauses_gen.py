...
stage1MsgOutPath = tmp_solution_path+f"{obj_}_boundary_out_print.txt"
try:
...
### Start file I/O
clause_file_name = tmp_solution_path + f'{obj_}_boundary_clauses_' + consts_path.split('/')[-1]
try:
...
if obj_=="mdms":
    ...
    write_clauses_to_file(f, b1.reshape(-1,1), SOFT_CLAUSE_W)
    print(f' ---> [obj] ---> b1: {b1.reshape(-1,1).shape} | ndc: {n_DC}')
### Base Hard Clauses
...
if obj_ == "mdms":
    ...
    write_clauses_to_file(f, clause_list_33, HARD_CLAUSE_W)
    tmp_time_counter_end = time.perf_counter()
    TC.counter(tmp_time_counter_start,
                tmp_time_counter_end,
                '33',
                clause_list_33.shape)
...
if use_SmartPair=='smart':
    ...
    command = ['java',
                    ...
                    f'-Dobj_={obj_}', 
                    '-jar', 
                    'smart-pair_allObj_j11.jar'] # smart-pair_obj_j11.jar  | smart-pair.jar
...
Final_Clauses_File_Name = tmp_solution_path + f'{obj_}_boundary_clauses_final'
...
# loandra solver
loandra_res_file_name = tmp_solution_path + f'{obj_}_boundary_loandra_res'  #f'{data_file_path.split("_")[-1]}_loandra_res'
