import numpy as np
import pandas as pd
import math,re, time, timeit,os,datetime
from itertools import combinations
from sklearn.cluster import KMeans
from sklearn.metrics import adjusted_rand_score


### Distance based Euc chain
# chain ml cl sep 2
def consts_euc_chain_sep_CL(df,CL, cl_var, HARD_CLAUSE_W, f):
  # cl chain
  ...
  # IO
  for chain_pair in cl_chain_join:
    f.write(str(HARD_CLAUSE_W)+' '+' '.join(str(t) for t in chain_pair) + ' 0\n')

def consts_euc_chain_sep_ML(df, ML, ml_var, HARD_CLAUSE_W, f):
  # ml chain
  ...
  # IO
  for chain_pair in ml_chain_join:
    f.write(str(HARD_CLAUSE_W)+' '+' '.join(str(t) for t in chain_pair) + ' 0\n')
...
### get the mlvar, clvar from out put res, if one flag is set to false, the corresponding
# var returned is False
def get_ml_cl_var_fromRes(out_res_path, ML_flag=False, CL_flag=False):
    # var_pattern = re.compile(r'^(ml:|cl:).*\d+$')
    ml_var_r = re.compile(r'ml:\s*(\d+)\s*-\s*(\d+)')
    cl_var_r = re.compile(r'cl:\s*(\d+)\s*-\s*(\d+)')
    ml_var, cl_var=np.array([], dtype='int'),np.array([], dtype='int')
    # get the vars for p2 from p2 output file
    with open(out_res_path, 'r') as f:
        lines = ' '.join(f.readlines())
        if ML_flag:
            match = ml_var_r.search(lines)
            ml_var = np.arange(int(match.group(1)),int(match.group(2))+1)
        if CL_flag:
            match = cl_var_r.search(lines)
            cl_var = np.arange(int(match.group(1)),int(match.group(2))+1)
    return ml_var, cl_var
...
def get_optimal_ml_cl(res_file_name, out_res_path, start_ind, ML=[], CL=[]):
  ML_final, CL_final = np.array([], dtype='int'),np.array([], dtype='int')
  # get res list
  ...
  if lines_list[-1][0] == 'v':
    ...
    return False
  # get ml and cl res
  ml_var, cl_var = get_ml_cl_var_fromRes(out_res_path, ML_flag=(len(ML)>0), CL_flag=(len(CL)>0))
  if len(ML)>0:
    ml_res = res_list[ml_var-1]
    ML_final_index = np.array(np.where(ml_res==1)[0])
    ML_final = np.array(ML)[ML_final_index,:]
  if len(CL)>0:
    cl_res = res_list[cl_var-1]
...
###### Print result output
def output_1p(loandra_res_file_name,TC,ML=[],ml_var=[], CL=[], cl_var=[]):
