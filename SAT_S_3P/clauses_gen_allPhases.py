...
use_Chain = sys.argv[6]
stage1_timeout = sys.argv[7]
stage2_timeout = sys.argv[8]
stage3_timeout = sys.argv[9]
...
# set base startind for stage-2 and stage-3, the previous variables are same
...
SOFT_CLAUSE_W = 1
HARD_CLAUSE_W = len(CL) + 1

# only CL is used as SOFT clause
Num_VARS = cl_var[-1]
...
############### Consts order enforecement
if use_Chain=="euc":
  # EUC chain on CL
  consts_euc_chain_sep_CL(df,CL, cl_var, HARD_CLAUSE_W, f)
###############

### CL as Obj
np.savetxt(f,
           np.hstack((
             np.repeat(SOFT_CLAUSE_W, len(cl_var)).reshape(-1, 1),
             cl_var.reshape(-1, 1),
             np.zeros(len(cl_var)).reshape(-1, 1)
           )),
           fmt='%d')
...
# @debug
print(f'[clause generation] -> finished at {time.perf_counter() - stage1_Total_time_counter_start}')
print(f'[loandra solver] -> started at {time.perf_counter() - stage1_Total_time_counter_start}')

### call loandra
stage1_loandra_res_file_name = tmp_solution_path + 'phase_1_loandra_res'
loandra_cmd = f'timeout {stage1_timeout}s ./loandra-master/loandra_static -pmreslin-cglim=30 -weight-strategy=1 -print-model -verbosity=1 ' \
              + final_clause_file_name + ' >' + stage1_loandra_res_file_name 
# print(loandra_cmd)
tmp_time_counter_start = time.perf_counter()
...
print(f'[loandra solver] -> finished at {time.perf_counter() - stage1_Total_time_counter_start}')
...
# read loandra res
loandra_status, _, cl_res = output_1p(stage1_loandra_res_file_name,TC, CL=CL, cl_var=cl_var)
...
# get optimal CL from previous step
print(f"original CL size: {CL.shape}")
_,CL = get_optimal_ml_cl(stage1_loandra_res_file_name, stage1MsgOutPath, start_ind, CL=CL)
print("optimal CL size: {CL.shape}")

# introduce new vars ml_var, cl_var, cl here based on optimal CL*
...
NUM_DISTANCE_CLASS = len(Distance_Class)
SOFT_CLAUSE_W = 1
HARD_CLAUSE_W = len(ML) + 1

Num_VARS = start_ind - 1

######################## Clause Gen ###############################
### Start file I/O
clause_file_name = tmp_solution_path + 'phase2_clauses_tmp'
try:
  os.remove(clause_file_name)
except OSError:
  pass
f = open(clause_file_name, "a") 

### ML as Obj Soft clauses CL as Hard clauses
np.savetxt(f ,
           np.hstack(
               (np.concatenate((np.repeat(HARD_CLAUSE_W, len(cl_var)), np.repeat(SOFT_CLAUSE_W, len(ml_var))), axis=0).reshape(-1, 1),
...
if use_Chain=="euc":
  # EUC chain
  consts_euc_chain_sep_ML(df, ML, ml_var, HARD_CLAUSE_W, f)
...
clauses_names = (['clause_list_7','clause_list_8','clause_list_9',
                    ...
                    'clause_list_19','clause_list_21', 
                    'clause_list_22','clause_list_23','clause_list_24',
...
tmp_time_counter_start = time.perf_counter()
### Add header to clause
final_clause_file_name = tmp_solution_path + 'phase_2_clauses_final'
with open(final_clause_file_name, 'w') as f0:
  with open(clause_file_name, 'r+') as f1:
    clauses_list_fr_file = f1.readlines()
    num_clauses = len(clauses_list_fr_file)
...
# delete the file without handler
os.remove(clause_file_name)
stage2_loandra_res_file_name = tmp_solution_path + 'phase_2_loandra_res'
...
loandra_cmd = f'timeout {stage2_timeout}s ./loandra-master/loandra_static -pmreslin-cglim=30 -weight-strategy=1 -print-model -verbosity=1 ' \
              + final_clause_file_name + ' >' + stage2_loandra_res_file_name 
# print(loandra_cmd)
...
loandra_status, ml_res, cl_res = output_1p(stage2_loandra_res_file_name,TC, CL=CL, cl_var=cl_var, ML=ML, ml_var=ml_var)
print(f"V0 ML:{ML.shape}-{np.sum(ml_res)}, CL: {CL.shape}-{np.sum(cl_res)}")
...
if loandra_status not in ["OPTIMUM FOUND", "SATISFIABLE"]:
    print(f"\n*** stage2 loandra status not valid: {loandra_status} ->exit***")
    stage2MsgOut.close()
    sys.exit()
...
TC = Time_Counter_v2()
stage3_Total_time_counter_start = time.perf_counter()
stage3MsgOutPath = tmp_solution_path+"phase_3_out_print.txt"
try:
    os.remove(stage3MsgOutPath)
except OSError:
    pass
stage3MsgOut = open(stage3MsgOutPath, "a")
sys.stdout = stage3MsgOut

# introduce variable for MD-MS obj
start_ind = baseStartInd
...
ML,_ = get_optimal_ml_cl(stage2_loandra_res_file_name, stage2MsgOutPath, start_ind, ML=ML)

############ Clause Gen #############

### Start file I/O
clause_file_name = tmp_solution_path + 'phase3_clauses_tmp'
try:
  os.remove(clause_file_name)
except OSError:
  pass
...
for ind, clause_list in enumerate([clause_list_7,clause_list_8,clause_list_9,
                    ...
                    clause_list_19,clause_list_21, clause_list_32_33_34]):
    write_clauses_to_file(f, clause_list, HARD_CLAUSE_W)
    stage3NumOfClauses.append(len(clause_list))
    print(f"{clauses_names[ind]} -size: {len(clause_list)}")
...
for sub_clause_list in clause_list_20:
    ...
    counter+=sub_clause_list.shape[0]
print(f"clauses20 -size:{counter}")
stage3NumOfClauses.append(counter)
...
tmp_time_counter_start = time.perf_counter()
### Add header to clause
final_clause_file_name = tmp_solution_path + 'phase_3_clauses_final'
with open(final_clause_file_name, 'w') as f0:
  with open(clause_file_name, 'r+') as f1:
    clauses_list_fr_file = f1.readlines()
    num_clauses = len(clauses_list_fr_file)
...
### call loandra
stage3_loandra_res_file_name = tmp_solution_path + 'phase_3_loandra_res'
loandra_cmd = f'timeout {stage3_timeout}s ./loandra-master/loandra_static -pmreslin-cglim=30 -weight-strategy=1 -print-model -verbosity=1 ' \
              + final_clause_file_name + ' >' + stage3_loandra_res_file_name 
# print(loandra_cmd)
...
stage3_Total_time_counter_end = time.perf_counter()
TC.counter(stage3_Total_time_counter_start,
           stage3_Total_time_counter_end,
           'Total',
           np.nan)
# print stat
TC.print_dict()
...
output_final_stage(stage3_loandra_res_file_name, b0,b1,x,y,n_points, n_labels)
